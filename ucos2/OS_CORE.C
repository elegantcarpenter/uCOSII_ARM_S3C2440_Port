...
void  OSStart (void)
{
    INT8U y;
    INT8U x;
//put_string("---OSStart----1\n\r");

    if (OSRunning == FALSE) {
//put_string("---OSStart----2\n\r");		
        y             = OSUnMapTbl[OSRdyGrp];        /* Find highest priority's task priority number   */
        x             = OSUnMapTbl[OSRdyTbl[y]];
//put_string("---OSStart----3\n\r");		
        OSPrioHighRdy = (INT8U)((y << 3) + x);
        OSPrioCur     = OSPrioHighRdy;

        OSTCBHighRdy  = OSTCBPrioTbl[OSPrioHighRdy]; /* Point to highest priority task ready to run    */
//put_string("---OSStart----4\n\r");				
        OSTCBCur      = OSTCBHighRdy;
//put_string("---OSStart----4-1\n\r");						
//put_string("---OSStart----4-2\n\r");				
//put_string("---OSStart----4-3\n\r");				
put_string("---OSStart----5\n\r");		
        OSStartHighRdy();                            /* Execute target specific code to start task     */	
put_string("---OSStart----6\n\r");		
    }
}
/*$PAGE*/
/*
...
#else
    #if OS_STK_GROWTH == 1
//put_string("----OS_InitTaskIdle----1\n\r");	
    (void)OSTaskCreate(OS_TaskIdle,
                       (void *)0,
                       &OSTaskIdleStk[OS_TASK_IDLE_STK_SIZE - 1],
                       OS_IDLE_PRIO);
//put_string("----OS_InitTaskIdle----2\n\r");
    #else
    (void)OSTaskCreate(OS_TaskIdle,
                       (void *)0,
                       &OSTaskIdleStk[0],
...
{
put_string("Enter OS_TaskIdle\n\r");	
#if OS_CRITICAL_METHOD == 3                      /* Allocate storage for CPU status register           */
    OS_CPU_SR  cpu_sr;
#endif    
//put_string("----OS_TaskIdle----1\n\r");	    
    
    pdata = pdata;                               /* Prevent compiler warning for not using 'pdata'     */
//put_string("----OS_TaskIdle----3\n");		
    for (;;) {
put_string("----OS_TaskIdle----1\n\r");
        OS_ENTER_CRITICAL();
put_string("----OS_TaskIdle----2\n\r");
        OSIdleCtr++;
put_string("----OS_TaskIdle----3\n\r");
        OS_EXIT_CRITICAL();
put_string("----OS_TaskIdle----4\n\r");
        OSTaskIdleHook();                        /* Call user definable HOOK                           */
put_string("----OS_TaskIdle----5\n\r");			
    }
}
/*$PAGE*/
/*
