...
void nand_read(unsigned char *buf, unsigned long nand_addr, int page_cnt)
{
	...
	//for(i=nand_addr; i<page_cnt; i++,nand_addr+=2048){
		for(i=nand_addr; i<50; i++,nand_addr+=2048){
		NFCMMD = 0x00;		
#if 0		
		NFADDR = (nand_addr)&(0xff);
		NFADDR = (nand_addr>>8)&(0x0f);
		NFADDR = (nand_addr>>12)&(0xff);
		NFADDR = (nand_addr>>20)&(0xff);
		NFADDR = (nand_addr>>28)&(0x01);
#else
NFADDR = (0x00);
NFADDR = (0x00);
NFADDR = ((i) & 0xff);
...
{
		put_string("Enter MainTask\r\n");
		timer_start();
	  while(1){
	  	put_string("MainTask-1\r\n");
	  	OSTimeDly(OS_TICKS_PER_SEC*1);
	  	put_string("MainTask-2\r\n");
	  }
}
...
void os_main(void)
{
  ...
  GPBDAT =  0;
	...
	OSTaskCreate(MainTask,(void *)0, &(MainTaskStk[256 - 1]), 9);

	OSStart();

put_string("----------- Tony system start-----------9\n");	
	#endif	
}
