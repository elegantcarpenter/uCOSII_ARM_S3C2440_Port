...
4\. SDRAM init，NAND Flash driver，CPU init，MMU init，Code
...
我寫的Code超過4K，並燒到Flash，而CPU只會COPY

4K。

解決方式: 將程式做Relocation

(1) 在4K範圍內將SDRAM initial做好，並將全部的
...
因Link Address為SDRAM的位址，故在function call或
...
先將當前被Premptive的Task的Registers值存到自

己的PCB，接著再將要執行的Task的Process Control Block
...
將當前被Premptive的Task的Registers值存到自己的
...
- 建立了三個Task
- 並彼此傳遞Message，分別採用block和noblock的方式
- 三個Task能正常的做Context Switch
- OS的Wait(Idle功能OK
- UART能印出訊息